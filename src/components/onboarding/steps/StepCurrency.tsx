...
import { ArrowLeft, ArrowRight, DollarSign, Wallet } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center space-y-6 py-2 w-full max-w-lg mx-auto">
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-muted-foreground">Isso nos ajuda a calibrar a Inteligência Artificial.</p>
      </div>
             ...
             {['BRL', 'USD', 'EUR'].map((curr) => (
               ...
               >
                 ...
                 <span className="text-xs font-medium opacity-80">
                   {curr === 'BRL' ? 'R$ Real' : curr === 'USD' ? '$ Dólar' : '€ Euro'}
                 </span>
               </div>
             ))}
          </div>
        ...
        <div className="space-y-3">
          ...
          <div className="relative">
             <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground font-bold text-lg">
               {field.value === 'BRL' ? 'R$' : field.value === 'EUR' ? '€' : '$'}
             </span>
             <Input 
              id="monthlyIncome"
              type="number"
